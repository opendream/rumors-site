...
const { defineType, createAction, createReducer } = createDuck(
  'editArticleDetail'
);
...
export const submitArticleCategories = params => dispatch => {
  ...
  NProgress.start();
  return gql`
    mutation($articleId: String!, $categories: [String]) {
      UpdateArticleCategories(id: $articleId, categories: $categories) {
        id
        categories
      }
    }
...
export const fetchArticleHyperlink = params => dispatch => {
  dispatch(
    setState({
      key: 'aticleHyperlinkLoading',
      value: params.hyperlink.get('url'),
    })
  );

  NProgress.start();
  return gql`
    mutation($articleId: String!, $url: String!) {
      UpdateArticleHyperlink(articleId: $articleId, url: $url) {
        id
      }
    }
...
export const fetchReplyHyperlink = params => dispatch => {
  ...
  NProgress.start();
  return gql`
    mutation($replyId: String!, $url: String!) {
      UpdateReplyHyperlink(replyId: $replyId, url: $url) {
        id
      }
    }
  `({ replyId: params.replyId, url: params.hyperlink.get('url') }).then(
    resp => {
